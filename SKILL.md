...
Phase G  简介PPT输出（python-pptx，专利路径默认）
```
...
输出策略：专利→交底书.docx+专利.docx(封面+权利要求+说明书+摘要)+简介.pptx；软著→2个.docx；交底书→1个.docx。
...
专利路径默认执行，其他路径`--ppt`触发。python-pptx工作流：`.temp/{案件名}-ppt.py`。

5-8页：封面→背景痛点→核心创新→系统架构→效果对比→布局→实施计划。16:9宽屏，科技蓝主题，微软雅黑字体。
